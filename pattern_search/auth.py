...
def get_total_base_payment_and_yesterday_increment():
    ...
    """
    with open(os.path.join(_CRED_DIR, "brain_token.txt"), encoding="utf-8") as f:
        token = f.read().strip()
    headers = {
        "Cookie": f"t={token}",
...
# `python credentials/token_refresh.py` in a separate terminal — it loops by
# default and refreshes the JWT on a 3h55m schedule. The miner just reads
# brain_token.txt and pauses if it ever finds the file missing or expired.
    ...
    with token_lock:
        ...
        while True:
            try:
                ...
                state.pause("no brain_token.txt — run token_refresh.py")
            ...
            print(f"⏸️ Token expired/invalid (expiry={expiry}s). "
                  f"Waiting {TOKEN_WAIT_POLL_SECONDS}s for token_refresh.py to renew it…")
            time.sleep(TOKEN_WAIT_POLL_SECONDS)
